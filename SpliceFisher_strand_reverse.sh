...
perl $codeDir/SpliceFisher_gene.pl -s r $codeDir/exon.unique.txt $bamFiles > $outputPrefix.gene.count.txt

type=exon
perl $codeDir/SpliceFisher_$type.pl -s r $codeDir/$type.unique.txt $outputPrefix.gene.count.txt $bamFiles > $outputPrefix.$type.count.txt
...
type=exon_pair
perl $codeDir/SpliceFisher_$type.pl -s r $codeDir/$type.txt $bamFiles > $outputPrefix.$type.count.txt
Rscript $codeDir/SpliceFisher.R $outputPrefix.$type.count.txt $outputPrefix.$type.txt
perl $codeDir/SpliceFisher_filter.pl $outputPrefix.$type.txt $alpha > $outputPrefix.$type.filtered.txt
