...
perl $directory/SpliceFisher_gene.pl -s r $directory/exon.unique.txt $bamFiles > $outputPrefix.gene.count.txt

type=exon
perl $directory/SpliceFisher_$type.pl -s r $directory/$type.unique.txt $outputPrefix.gene.count.txt $bamFiles > $outputPrefix.$type.count.txt
...
type=exon_pair
perl $directory/SpliceFisher_$type.pl -s r $directory/$type.txt $bamFiles > $outputPrefix.$type.count.txt
Rscript $directory/SpliceFisher.R $outputPrefix.$type.count.txt $outputPrefix.$type.txt
perl $directory/SpliceFisher_filter.pl $outputPrefix.$type.txt $alpha > $outputPrefix.$type.filtered.txt
